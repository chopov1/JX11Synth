...
//==============================================================================
MyJX11AudioProcessor::MyJX11AudioProcessor()
...
#endif
{
    //store the value of the pointer to the nosie parameter, so we dont need to look up the value using the id string in process block
    noiseParam = dynamic_cast<juce::AudioParameterFloat*>(apvts.getParameter(ParameterID::noise.getParamID()));

    //looking up the parameters using the templated function from the Util header, so we dont need to type dynamic_cast<> shit all the time
...
//==============================================================================
const juce::String MyJX11AudioProcessor::getName() const
{
    return JucePlugin_Name;
}

bool MyJX11AudioProcessor::acceptsMidi() const
{
...
bool MyJX11AudioProcessor::producesMidi() const
{
...
bool MyJX11AudioProcessor::isMidiEffect() const
{
   #if JucePlugin_IsMidiEffect
    return true;
   #else
...
double MyJX11AudioProcessor::getTailLengthSeconds() const
{
    return 0.0;
}

int MyJX11AudioProcessor::getNumPrograms()
{
    return int(presets.size());   // NB: some hosts don't cope very well if you tell them there are 0 programs,
                // so this should be at least 1, even if you're not really implementing programs.
}

int MyJX11AudioProcessor::getCurrentProgram()
{
    return currentProgram;
}

void MyJX11AudioProcessor::setCurrentProgram (int index)
{
...
const juce::String MyJX11AudioProcessor::getProgramName (int index)
{
    return { presets[index].name};
}

void MyJX11AudioProcessor::changeProgramName (int /*index*/, const juce::String& /*newName*/)
{
    //names of argyuments are commented to stop compiler wanring about unused arguyments
}

//==============================================================================
void MyJX11AudioProcessor::prepareToPlay (double sampleRate, int samplesPerBlock)
{
...
void MyJX11AudioProcessor::releaseResources()
{
...
void MyJX11AudioProcessor::reset()
{
    synth.reset();
}

#ifndef JucePlugin_PreferredChannelConfigurations
bool MyJX11AudioProcessor::isBusesLayoutSupported (const BusesLayout& layouts) const
{
  #if JucePlugin_IsMidiEffect
...
void MyJX11AudioProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
    ...
    bool expected = true;
    //the compare exchange is like "check if true set back to false" it is a single atomic operation that either succeeds or fails. if it fails another thread was trying to write parametersChanged while processBlock was trying to read it. If it fails no big deal hopefully it will be free on the next processBlock call
    //is nonrealtime explained a bit on pg 175 of c++ synth book
...
void MyJX11AudioProcessor::splitBufferByEvents(juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
     int bufferOffset = 0;
...
void MyJX11AudioProcessor::handleMIDI(uint8_t data0, uint8_t data1, uint8_t data2)
{
    //program change - supports changing the preset via midi message. need to send a program change midi message. 
    if ((data0 & 0xF0) == 0xC0) {
...
void MyJX11AudioProcessor::render(juce::AudioBuffer<float>& buffer, int sampleCount, int bufferOffset)
{
...
//==============================================================================
bool MyJX11AudioProcessor::hasEditor() const
{
    return true; // (change this to false if you choose to not supply an editor)
}

juce::AudioProcessorEditor* MyJX11AudioProcessor::createEditor()
{
...
//==============================================================================
void MyJX11AudioProcessor::getStateInformation (juce::MemoryBlock& destData)
{
...
void MyJX11AudioProcessor::setStateInformation (const void* data, int sizeInBytes)
{
...
void MyJX11AudioProcessor::update()
{
   ...
   //this one uses a pointer, much faster
...
{
    //emplace back constructs and adds a new element to a vector
...
juce::AudioProcessorValueTreeState::ParameterLayout MyJX11AudioProcessor::createParameterLayout()
{
...
{
    return new MyJX11AudioProcessor();
}
